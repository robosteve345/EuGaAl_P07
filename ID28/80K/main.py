...
# assign directory
directory = '/Users/stevengebel/PycharmProjects/EuGaAl_P07/ID28/80K'
...
for item in os.listdir(directory):
    if not item.startswith('.') and item.endswith('.img') and os.path.isfile(os.path.join(directory, item)):
        ...
        plt.imshow(obj.data, cmap='viridis', norm = LogNorm(vmin=0.1, vmax=np.max(data)))
        # INTENSITY MAP
        ...
        ax.grid(True, which='both',axis='both',linestyle='-', color='black', lw=0.1)
        ax.set_xlabel('H (rlu)')
        ax.set_ylabel('L (rlu)')
        ax.set_title("{}".format(item))

        fig.colorbar(im, ax=ax)
        plt.savefig("/Users/stevengebel/PycharmProjects/EuGaAl_P07/ID28/80K/{}.jpg".format(item), dpi=400)
