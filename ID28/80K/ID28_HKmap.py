...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 30 20:02:43 2023
...
"""Visualization of integrated Intensity HK-maps with linecutting for I4/mmm
Eu(Ga,Al)4
...
###########################################################
# INPUT
latticeparam1, latticeparam2, resolution = np.loadtxt("80K.txt", 
                                           usecols=(0, 1, 2), 
...
#             INPUT FOR LINECUT
# =============================================================================
xzoomindex = 8  #  Linecut integer H value  -->indecesx
yzoomindex = 8  #  Zoom-window centered K-cut value   -->indecesy
st = time.time()
for item in os.listdir(directory):
# =============================================================================
#     HK MAPS
# =============================================================================
    if not item.startswith('.') and item.startswith('pno1_HK') and item.endswith('0.5A.img') and os.path.isfile(os.path.join(directory, item)):
              print("CURRENT FILE: {}".format(item))
              # print("Header: {}".format(fabio.open(item).header))
              #################################################################
              # INPUT FOR KL-maps
              img = item
              lat_a, lat_b = float(latticeparam1), float(latticeparam1)
              #################################################################
              obj = fabio.open(img)
              data = obj.data
              NX = obj.header["Pixels in x"]
              NY = obj.header["Pixels in y"]
              qmaxa, qmaxc = lat_a/(resolution), lat_a/(resolution)
              ...
              xlim_pos, xlim_neg, ylim_pos, ylim_neg = int(qmaxa), \
                  -int(qmaxa), int(qmaxa), -int(qmaxa)
      # =======================================================================
      #         Compute Linecuts
      # =======================================================================
              fig = plt.figure(figsize=(6, 8))
              plt.title("{}, mean of ±{}-pixel n.-neighbors".format(img, nmax))
      ...
      # =======================================================================
                  X_detail = qyrange[indicesy[yzoomindex]-30:indicesy[yzoomindex]+30]
                  ...
                  sub_axes.plot(X_detail, Y_detail, marker='s', ms=1, lw=0.5, 
                      label="[{}{}L0]".format(round(qxrange[i]),
      ...
      # =======================================================================
              ...
              plt.ylim(qyrange[indicesy[14]], qyrange[indicesy[-12]]);
              plt.xlim(qxrange[indicesx[2]], qxrange[indicesx[-4]]);
              plt.xlabel('H (r.l.u.)');
              plt.ylabel('K (r.l.u.)');
              plt.colorbar();
