...
def main():
    ...
    for spectrum in sorted_spectra:
        ...
        if temperatureingabe == "y":
            ...
            pass
        saveplot = input("Plot speichern? (y/n)")
        if saveplot == "y":
            plt.savefig("{}.jpg".format(filename), dpi=500)
        else:
            pass
    speicherueberpruefung = input("Datensatz speichern? (y/n): ")
    if speicherueberpruefung == "y":
        np.savetxt(r"datalist.txt",
                   np.transpose([R1, dR1, FWHM2, dFWHM2, R2, dR2, FWHM1, dFWHM1, A1, dA1, A2, dA2, B, dB]),
                   header='T_manual(K) T_weinstein  R1  dR1 FWHM_R1  dFWHM_R1 R2 dR2 FWHM_R2  dFWHM_R2 A1  dA1 A2  dA2  B  dB')
    else:
        pass
    plt.errorbar(T_manual, R1, yerr=dR1, c='b', label=r'R$_1$', marker='.', ls='', capsize=3)
    plt.errorbar(T_manual, R2, yerr=dR2, c='g', label='R$_2$', marker='.', ls='', capsize=3)
